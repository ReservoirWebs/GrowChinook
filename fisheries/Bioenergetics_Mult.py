...
def GetVals(Light, Total_Daphnia, DaphSize, Site, Month, Year):
    FallCreeklength = dict(
        [('March', 31), ('April', 30), ('May', 31), ('June', 30), ('July', 31), ('August', 31), ('September', 30)])
    HillsCreeklength = dict(
        [('March', 31), ('April', 30), ('May', 31), ('June', 30), ('July', 31), ('August', 31), ('September', 30)])
    LookoutPointlength = dict(
        [('March', 31), ('April', 30), ('May', 31), ('June', 30), ('July', 31), ('August', 31), ('September', 30)])
    ...
    FCd = dict([('March', 620.83), ('April', 7220.57), ('May', 10930.27), ('June', 5020.65), ('July', 2010.06),
                ('August', 1940.78)])
    HCd = dict([('March', 120.57), ('April', 170.48), ('May', 5780.05), ('June', 59310.33), ('July', 4140.69),
                ('August', 3390.29)])
    LPd = dict([('March', 30.14), ('April', 40.91), ('May', 5780.05), ('June', 21110.15), ('July', 7030.72),
                ('August', 13820.30)])
    ...
    FCsize = dict(
        [('March', 1.207), ('April', 1.248), ('May', 1.139), ('June', 1.305), ('July', 1.514), ('August', 1.145)])
    HCsize = dict(
        [('March', 1.455), ('April', 1.077), ('May', 1.053), ('June', 1.045), ('July', 1.769), ('August', 1.516)])
    LPsize = dict(
        [('March', 1.457), ('April', 0.957), ('May', 1.064), ('June', 1.247), ('July', 1.664), ('August', 2.002)])
...
class Batch:
    def __init__(self, Site, Month, Year, Light, DaphSize, TotalDaphnia, StartingMass, Dmax, Dmin, Tmax, Tmin,
                 TempCurve, DYear, DMonth, DSite):
        self.Site = Site
        self.Month = Month
        self.Year = Year
        self.Light = Light
        ...
        self.out = {}
        self.DaphWeightdry = (exp(1.468 + 2.83 * log(
            self.DaphSize))) / 1000000  # Based of Cornell equation (g) #WetDaphWeight <- DaphWeight*(8.7/0.322) #From Ghazy, others use ~10%
        ...
        print("tempfile: %s/n" % temperature_file)
    ...
    def compute_growth(self, consumption, prey, preyenergy, egestion, excretion, SDAction, respiration, predatorenergy,
                       W):
        consumptionjoules = consumption * inner(prey, preyenergy)
        return (consumptionjoules - (
        (egestion + excretion + SDAction) * inner(prey, preyenergy) + respiration * self.O2Conv)) / predatorenergy * W
    ...
    def plot_growth():
        ...
        gs_d = [growth_fn(d, self.out['StartingLength'][0], self.out['StartingMass'][0], day_hours, DayLight, self.prey)
                for d in depth_arr]
        gs_n = [
            growth_fn(d, self.out['StartingLength'][0], self.out['StartingMass'][0], night_hours, NightLight, self.prey)
            for d in depth_arr]
    ...
    def growth_fn(self, depth, StartingLength, StartingMass, hours, light, prey):
        ...
        foraging = self.compute_foragingbydepth(StartingLength, StartingMass, light, self.daphline, self.daph_auc,
                                                depth) * hours
        ft = self.compute_ft(temp)
        cmax = self.compute_cmax(StartingMass)
        P = min(foraging / cmax, 1)
        (consumption, egestion, excretion, respiration, SDAction) = self.compute_bioenergetics(StartingMass, temp, P,
                                                                                               self.prey,
                                                                                               self.digestibility)
        day_proportion = hours / 24.0
        consumption *= day_proportion
        respiration *= day_proportion
        growth = self.compute_growth(consumption, prey, self.preyenergy, egestion, excretion, SDAction, respiration,
                                     self.predatorenergy, StartingMass)
        return (growth, consumption)

    def Run_Batch(self):
        daylength = dict(
            [('March', 11.83), ('April', 13.4), ('May', 14.73), ('June', 15.42), ('July', 15.12), ('August', 13.97),
             ('September', 12.45)])
        ...
        for d in range(ndays):
            (day_depth, day_growth, day_consumption) = self.best_depth(self.StartingLength, self.StartingMass,
                                                                       day_hours, self.DayLight, self.Depths)
            (night_depth, night_growth, night_consumption) = self.best_depth(self.StartingLength, self.StartingMass,
            ...
            if growth > 0:
                self.StartingLength = (self.StartingMass / 0.0003) ** (
                1 / 2.217)  # weight to fork length (MacFarlane and Norton 2008)
        ...
        condition = float(100 * self.StartingMass * ((self.StartingLength / 10) ** (-3.0)))
        return (self.out['day_depth'],self.out['night_depth'],self.out['growth'],self.out['StartingMass'],self.out['StartingLength'])
